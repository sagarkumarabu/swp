  ...
  solids.forEach(solid => {
    ...
    swatch.onclick = () => {
      ...
      if (target) {
        ...
        // Update all areas in the areas array
        if (typeof areas !== 'undefined' && Array.isArray(areas)) {
          areas.forEach(area => {
            if (target.value === 'doorShade') {
              area.doorShade = solid.color;
            } else if (target.value === 'pilasterShade') {
              area.pilasterShade = solid.color;
            } else if (target.value === 'topRailShade') {
              area.topRailShade = solid.color;
            }
          });
        }
        // Update currentArea if it exists
